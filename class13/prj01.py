...
async def weather(interaction: discord.Interaction, city: str, forecast: bool = False, ai: bool = False):
    ...
    if "list" not in raw_forecast:
        await interaction.followup.send("無法取得天氣預報資料，請確認城市名稱是否正確。")
        return
    ...
    try:
        response = openai.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"以下是{city_name}未來幾天的天氣預報資料，請根據這些數據提供詳細的分析和解釋：\n{raw_forecast}"
                },
            ],
            temperature=0.2,
        )
        ...
        await interaction.followup.send("分析天氣預報資料時發生錯誤，請稍後再試。")
